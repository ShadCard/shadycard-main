...
async function ensureDepositsTelegramMessageColumn() {
  ...
  try {
    await db.execute(sql`
      ...
      ADD COLUMN telegram_message_id INT NULL
    `);
  } catch (err: any) {
    // MySQL errno 1060 = duplicate column (already exists) — safe to ignore
    if (err && (err.errno !== 1060) && !/Duplicate column name/i.test(String(err?.message || ""))) {
      console.warn("ensureDepositsTelegramMessageColumn: ignoring error", err?.message || err);
    }
  }
...
router.get("/admin/dashboard", requireAdmin, async (_req, res) => {
  const [u] = await db.select({ c: sql<number>`count(*)` }).from(usersTable);
  const [p] = await db
    .select({ c: sql<number>`count(*)` })
    .from(productsTable)
    .where(eq(productsTable.available, true));
  const [oTotal] = await db.select({ c: sql<number>`count(*)` }).from(ordersTable);
  ...
  const [sales] = await db
    .select({ s: sql<string>`coalesce(sum(total_usd),0)` })
    .from(ordersTable)
    .where(eq(ordersTable.status, "accept"));
  const [cost] = await db
    .select({ s: sql<string>`coalesce(sum(cost_usd),0)` })
    .from(ordersTable)
    .where(eq(ordersTable.status, "accept"));
  const [bal] = await db.select({ s: sql<string>`coalesce(sum(balance_usd),0)` }).from(usersTable);
  ...
  // 7-day chart (MySQL: use a derived table of 7 days instead of generate_series)
  const chart = await db.execute(sql`
    SELECT DATE_FORMAT(d, '%Y-%m-%d') as date,
      coalesce(sum(case when o.status='accept' then o.total_usd else 0 end), 0) as sales
    FROM (
...
async function hasColumn(tableName: string, columnName: string): Promise<boolean> {
  ...
  // MySQL: use INFORMATION_SCHEMA.TABLES filtered by the current database
  const dbName = process.env.MYSQL_DATABASE || (process.env.DATABASE_URL || "").split("/").pop()?.split("?")[0] || "";
...
router.get("/admin/users", requireAdmin, async (req, res) => {
  ...
  if (q) {
    rows = await db
      .select()
      .from(usersTable)
      .where(sql`${usersTable.username} LIKE ${"%" + q + "%"} OR ${usersTable.telegramId} LIKE ${"%" + q + "%"}`)
...
router.get("/admin/reports/sales", requireAdmin, async (_req, res) => {
  const rows = await db.execute(sql`
    SELECT DATE_FORMAT(DATE(created_at), '%Y-%m-%d') as date,
      ...
      coalesce(sum(case when status='accept' then total_usd else 0 end), 0) as revenue,
      coalesce(sum(case when status='accept' then cost_usd else 0 end), 0) as cost,
      coalesce(sum(case when status='accept' then total_usd - cost_usd else 0 end), 0) as profit
    FROM orders
...
router.get("/admin/reports/profit-log", requireAdmin, async (_req, res) => {
  const rows = await db.execute(sql`
    SELECT DATE_FORMAT(DATE(created_at), '%Y-%m-%d') as date,
      coalesce(sum(case when status='accept' then total_usd - cost_usd else 0 end), 0) as profit_usd,
      sum(case when status='accept' then 1 else 0 end) as accepted_orders
    FROM orders
    GROUP BY DATE(created_at)
    ORDER BY DATE(created_at) DESC
    LIMIT 90
...
router.post("/admin/backup", requireAdmin, async (_req, res) => {
  ...
  res.json({
    ok: true,
    filename: `shadycard-backup-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.sql`,
...
router.post("/admin/2fa/enable", requireAdmin, async (req, res) => {
  ...
  await db
    ...
    .where(eq(adminsTable.id, req.session.adminId!));
  res.json({ secret, otpauthUrl: `otpauth://totp/ShadyCard?secret=${secret}&issuer=ShadyCard` });
});
...
router.get("/admin/reports", requireAdmin, async (req, res) => {
  ...
  const dailyRows = await db.execute(sql`
    SELECT DATE_FORMAT(DATE(created_at), '%Y-%m-%d') as date,
      coalesce(sum(case when status='accept' then total_usd else 0 end), 0) as "salesUsd",
      coalesce(sum(case when status='accept' then total_usd - cost_usd else 0 end), 0) as "profitUsd",
      sum(case when status='accept' then 1 else 0 end) as "ordersCount"
    FROM orders
  ...
  const totResult = await db.execute(sql`
    SELECT
      coalesce(sum(case when status='accept' then total_usd else 0 end), 0) as "totalSalesUsd",
      coalesce(sum(case when status='accept' then total_usd - cost_usd else 0 end), 0) as "totalProfitUsd",
      sum(case when status='accept' then 1 else 0 end) as "orderCount"
    FROM orders
    WHERE (${from} IS NULL OR DATE(created_at) >= ${from})
      AND (${to} IS NULL OR DATE(created_at) <= ${to})
  `);
      ...
      if (existingProdId) {
        ...
        await db.execute(sql`
          UPDATE products SET
            ...
            quantity_type = ${quantityInfo.quantityType},
            quantity_values = ${quantityInfo.quantityValues ? JSON.stringify(quantityInfo.quantityValues) : null},
            source = 'provider'
          WHERE id = ${existingProdId}
        `);
        // Note: For MySQL JSON column, mysql2 driver auto-serializes objects/arrays, but
        // when passing a pre-stringified string we use CAST(... AS JSON) inside SQL.
        // The above is safe because mysql2 will send the value as a JSON-compatible string.
        updated++;
      }
