...
export default function Layout({
  ...
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem("shady-admin-theme") === "dark");
  const navigate = useNavigate();

  useEffect(() => {
    document.documentElement.classList.toggle("admin-dark", darkMode);
    localStorage.setItem("shady-admin-theme", darkMode ? "dark" : "light");
  }, [darkMode]);
    ...
    <div className="flex min-h-screen bg-slate-100" dir="rtl">
      ...
      >
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <div>
            <div className="text-lg font-bold text-brand-600">ShadyCard</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-slate-200 px-4 lg:px-6 py-3 flex items-center justify-between">
          ...
          <div className="hidden lg:block text-sm text-slate-500">مرحبًا بك في لوحة إدارة ShadyCard</div>
