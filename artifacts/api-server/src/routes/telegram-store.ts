...
const SUPPORT_URL = (process.env.TELEGRAM_SUPPORT_URL || "https://t.me/ShadyCardSupport").replace("@", "");
...
function fullNameFromTelegramUser(user: any): string {
  ...
  return fullName || user?.username || "ShadyUser";
}
...
async function sendAccountInfo(chatId: string | number, telegramUser: any) {
  ...
  const [stats] = await db
    .select({
      ...
      spentUsd: sql<number>`coalesce(sum(${ordersTable.totalUsd}), 0)`,
    })
    .from(ordersTable)
...
async function broadcastToStoreUsers(chatId: string | number, telegramUser: any, message: string) {
  ...
  let sent = 0;
  let failed = 0;
  const broadcastText = `📢 إعلان من ShadyCard\n\n${message.trim()}`;
