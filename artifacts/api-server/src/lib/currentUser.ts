...
import { resolveWebUser } from "../routes/auth.js";

const DEFAULT_TELEGRAM_ID = "8333183867";
const DEFAULT_USERNAME = "ShadyUser";
...
function identityError(message = "identity_missing"): never {
  ...
  err.statusCode = 401;
  err.publicMessage = "الهوية غير متاحة. سجّل الدخول أو افتح المتجر من داخل Telegram Mini App.";
  throw err;
}
...
function invalidIdentityError(): never {
  ...
  err.statusCode = 401;
  err.publicMessage = "تعذّر التحقق من هوية تيليجرام. أعد فتح المتجر من زر البوت داخل Telegram.";
  throw err;
}
...
/**
 * Resolve the current user, supporting BOTH:
 *   1. Web JWT token (Authorization: Bearer ... or shady_web_token cookie)
 *   2. Telegram identity (x-telegram-init-data header or x-telegram-id header)
 *
 * Web JWT takes precedence — if a valid token is present we use it.
 * Otherwise we fall back to Telegram identity.
 */
export async function getOrCreateCurrentUser(req?: Request) {
  // 1) Try web auth first
  const webUser = await resolveWebUser(req!).catch(() => null);
  if (webUser) return webUser;

  // 2) Fall back to Telegram identity
...
export async function getOrCreateCurrentUserStrict(req?: Request) {
  ...
  // 2) Fall back to Telegram identity (strict)
