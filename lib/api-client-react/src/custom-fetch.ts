...
const TG_CACHE_KEY = "shady_telegram_identity";
...
export async function customFetch<T = unknown>(
  ...
  // ShadyCard — Web auth: if not in Telegram mode, attach web JWT from localStorage
  if (!headers.has("authorization") && !headers.has("x-telegram-id")) {
    try {
      const webToken = globalThis?.localStorage?.getItem("shadycard_web_token");
      if (webToken) {
  ...
  // Include cookies for cross-origin requests when credentials are allowed
